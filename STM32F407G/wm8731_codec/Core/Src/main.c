...
int main ()
{
    ...
    //    HAL_GPIO_TogglePin(LD3_GPIO_Port, LD3_Pin);_
    HAL_Delay(1000);
    #else
...
{
    HAL_StatusTypeDef result = HAL_I2C_Master_Transmit(I2C_TX_handle,
...
{
    HAL_StatusTypeDef result = HAL_I2C_Master_Transmit(I2C_RX_handle,
                                    device_address, data, length, 10);
    if (result != HAL_OK)
    {
        return false;
...
bool WM8731_write(uint8_t reg, uint16_t val) {
    ...
    */
    ...
    bool result = I2C_TX_wI2C_TX_writerite(WM8731_I2C_ADDR, data, TRANSMIT_LENGTH);
    return result;
}
